...
<template>
  <v-main class="list">
    ...
    <v-card>
      ...
      <v-data-table :headers="headers" :items="reservasis" :search="search">
        <template v-slot:[`item.actions`]="{ item }">
          ...
          <v-icon medium @click="deleteHandler(item.id_reservasi)" color="red">
            mdi-delete
          </v-icon>
          <v-icon
            medium
            class="ml-5"
            @click="qrdialogOpen(item.id_reservasi)"
            color="green"
          >
            mdi-printer
          </v-icon>
    ...
    <v-dialog v-model="qrdialog" persistent max-width="450px">
      <v-card>
        <v-card-title>
          <span class="headline">Silahkan Scan QR Code !!</span>
        </v-card-title>
        <v-card-text>
          <qriously :value="qrid" :size="355" />
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="qrdialog = false">
            Cancel
          </v-btn>
        </v-card-actions>
      </v-card>
